...
今回の確認では、上記品質軸に対する新規の重大な未充足は見当たらなかった。
...
- 残るリスクは plan の妥当性ではなく、validator と test へどう素直に実装するかにある

## Handoff Notes

- 実装担当は、この計画を `validate_template.py` と `tests/test_template_contract.py` に落とし込む段階へ進めてよい
