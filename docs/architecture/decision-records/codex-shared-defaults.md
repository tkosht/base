...
- ただし `sandbox_mode = "workspace-write"` と `network_access = false` を維持し、`danger-full-access` は既定にしない
- この設定は maintainer-local convenience ではなく、review 済みの shared setting として docs に明記する
- generated repo は bootstrap 時に `.codex/config.toml` を見直し、自分たちの threat model に合わせて tighten するか、必要なら明示的に緩和する
