...
- `force_deleted_branches`: ユーザーの明示承認後に `git branch -D` で削除したローカルブランチ
...
6. 削除対象ブランチを抽出する。
   ...
   - squash merge 済みブランチは `git branch --merged <target_branch>` に出ないため、`git branch -vv` で upstream が gone のローカルブランチを抽出し、`gh pr list --state merged --search "head:<branch>" --json number,state,mergedAt,headRefName` で PR merge を確認できたものを `force_delete_candidates` に記録する。
...
8. 明示承認済みの force delete 候補を削除する。
   - ユーザーが `force_delete_candidates` を不要ブランチとして削除するよう明示した場合だけ実行する。
   - 実行直前に PR merge、remote branch gone、worktree 不在を再確認する。
   - 条件を満たす候補ごとに `git branch -D <branch>` を実行し、`force_deleted_branches` に記録する。
