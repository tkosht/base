...
```bash
uv run python .agents/skills/codex-subagent/scripts/codex_query.py --list --limit 5
```
**期待結果**: テーブル形式、5件以下表示
...
```bash
uv run python .agents/skills/codex-subagent/scripts/codex_query.py --list --mode pipeline --limit 5
```
...
uv run python .agents/skills/codex-subagent/scripts/codex_feedback.py --scope auto \
  --run-id $(uv run python .agents/skills/codex-subagent/scripts/codex_query.py \
